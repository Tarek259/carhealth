...
const Reports = {
    ...
    generatePrintableReport(carId) {
        ...
        return {
            ...
            odometerHistory: carReport.odometerHistory
        };
    },
    ...
    /**
     * Get spending by type chart data
     */
    getTypeChartData() {
        const stats = this.getUserStats();
        if (!stats) return null;
        ...
        const colors = {
            'تغيير زيت المحرك': '#3b82f6',
            'تغيير الإطارات': '#10b981',
            'تغيير البطارية': '#f59e0b',
            'تغيير الفرامل': '#ef4444',
            'تغيير سير الكاتينة': '#8b5cf6',
            'صيانة دورية': '#6b7280'
        };

        return Object.entries(stats.spendingByType).map(([type, amount]) => ({
            type,
            amount,

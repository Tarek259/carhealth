...
const Auth = {
	...
	/**
	 * Register remotely via API (/api/auth/register) if available
	 */
	async registerRemote(userData) {
		try {
			...
			if (!data.success) return { success: false, error: data.error || 'Remote register failed' };

			// Store in local storage for offline
			const users = this.getUsers();
			users[data.user.id] = { ...data.user, cars: data.user.cars || [], settings: data.user.settings || {} };
			this.saveUsers(users);
	...
	/**
	 * Login remotely via API (/api/auth/login) if available
	 */

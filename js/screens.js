...
const Screens = {
	...
	async handleLogin() {
		...
		// Try remote login first, fallback to local
		let result = await Auth.loginRemote(username, password).catch(() => ({ success: false }));
		if (!result.success) {
	...
	async handleRegister() {
		...
		// Try remote register first, fallback to local
		let result = await Auth.registerRemote({ fullName, username, email, phone, password }).catch(() => ({ success: false }));
		if (!result.success) {
			result = Auth.register({ fullName, username, email, phone, password });
		}
		if (result.success) {
			// Auto login after registration
			await Auth.loginRemote(username, password).catch(() => Auth.login(username, password));
